define("WatbAccountFileWithDocTypeDetail", ["css!WatbAccountFileSchemaDetailCSS"], function () {
	return {
		...
		methods: {
			...
			saveFile: function (selectedDocumentType, files) {
				...
				this.set("WatbDocumentType", { value: selectedDocumentType.value, displayValue: selectedDocumentType.displayValue });
			...
			onFileComplete: function (error, xhr, file, options) {
				if (!error) {
					...
					updateQuery.setParameterValue("WatbDocumentType", this.get("WatbDocumentType").value, this.Terrasoft.DataValueType.LOOKUP);
